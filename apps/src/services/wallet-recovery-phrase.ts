...
// App recovery dictionary. This is intentionally fixed and deterministic so
// generated phrases can be restored by the app parser.
...
function ensureCryptoRandomValues() {
  ...
  if (typeof globalWithCrypto.crypto?.getRandomValues !== "function") {
    throw new Error("crypto.getRandomValues is unavailable for recovery phrase generation.");
  }

  return globalWithCrypto.crypto.getRandomValues.bind(globalWithCrypto.crypto);
}
...
export function generateRecoveryPhrase(wordCount = 12) {
  if (wordCount < 12) {
    throw new Error("Recovery phrase must use at least 12 words.");
  }

  const fillRandom = ensureCryptoRandomValues();
